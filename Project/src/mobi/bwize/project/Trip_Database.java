...
public class Trip_Database {
	...
	public Cursor getAllTrips() {
		...
		// ArrayList<String> current_trips = new ArrayList<String>();
		...
		/*
		 * int itripcode = c.getColumnIndex(KEY_TRIP_ID); int
		 * idestination_country = c.getColumnIndex(KEY_DESTINATION_COUNTRY); int
		 * idestination_city = c.getColumnIndex(KEY_DESTINATION_CITY); int
		 * ideparture_code = c.getColumnIndex(KEY_DEPARTURE_FLIGHT_CODE); int
		 * ideparture_date = c.getColumnIndex(KEY_DEPARTURE_DATE); int
		 * ideparture_time = c.getColumnIndex(KEY_DEPARTUE_TIME); int
		 * ideparture_connection = c
		 * .getColumnIndex(KEY_DEPARTURE_FLIGHT_CONNECTION); int ivaccination =
		 * c.getColumnIndex(KEY_VACCINATION); int ivisa =
		 * c.getColumnIndex(KEY_VISA); int ireturn_country =
		 * c.getColumnIndex(KEY_RETURN_FLIGHT_CODE); int ireturn_city =
		 * c.getColumnIndex(KEY_RETURN_DATE); int ireturn_code =
		 * c.getColumnIndex(KEY_RETURN_TIME); int ireturn_date =
		 * c.getColumnIndex(KEY_RETURN_FLIGHT_CONNECTION); int ireturn_time =
		 * c.getColumnIndex(KEY_RETURN_DATE); int ireturn_connection =
		 * c.getColumnIndex(KEY_RETURN_TIME); int inotification_status =
		 * c.getColumnIndex(NOTIFICATION_STATUS); int icomments =
		 * c.getColumnIndex(KEY_COMMENTS);
		 * 
		 * for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) { String row
		 * = new String(); row = c.getString(itripcode) + " " +
		 * c.getString(idestination_country) + " " +
		 * c.getString(idestination_city) + " " + c.getString(ideparture_code) +
		 * " " + c.getString(ideparture_date) + " " +
		 * c.getString(ideparture_time) + " " +
		 * c.getString(ideparture_connection) + " " + c.getString(ivaccination)
		 * + " " + c.getString(ivisa) + " " + c.getString(ireturn_country) + " "
		 * + c.getString(ireturn_city) + " " + c.getString(ireturn_code) + " " +
		 * c.getString(ireturn_date) + " " + c.getString(ireturn_time) + " " +
		 * c.getString(ireturn_connection) + " " +
		 * c.getString(inotification_status) + " " + c.getString(icomments);
		 * 
	...
	public Cursor get_DashboardInfoTrips() {
		...
		ArrayList<String> dashboardinfo_trips = new ArrayList<String>();
		...
		for (c.moveToFirst(); !c.isAfterLast(); c.moveToNext()) {
			...
			Log.d("country",c.getString(idestination_country));
